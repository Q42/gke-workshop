...
deploy-kube-state-metrics:
	kubectl apply -f ./configs/kube-state-metrics
deploy-stuff:
...
build:
	...
	docker build -t gcr.io/$(PROJECT_ID)/code-only-istio-with-errorreporting:1.0 ./code/code-only-istio-with-errorreporting
...
push:
	...
	docker push gcr.io/$(PROJECT_ID)/code-only-istio-with-errorreporting:1.0
